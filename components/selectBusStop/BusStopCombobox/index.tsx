            ...
            <PopoverContent className="w-[var(--radix-popover-trigger-width)] p-0">
                <Command shouldFilter={false}>
                    <CommandInput
                        ...
                        value={search}
                        onValueChange={(value) => handleSearch({ target: { value } } as React.ChangeEvent<HTMLInputElement>)}
                    />
                    <CommandList>
                        <CommandEmpty className="py-2 text-center text-sm text-muted-foreground">{isLoading ? "Searching..." : ""}</CommandEmpty>
                        <CommandGroup>
                            {busStops.map((stop: BusStop) => (
                                ...
                                >
                                    <Check
                                        className={cn(
                                            "mr-2 h-4 w-4",
                                            value?.BusStopCode === stop.BusStopCode ? "opacity-100" : "opacity-0"
                                        )}
                                    />
